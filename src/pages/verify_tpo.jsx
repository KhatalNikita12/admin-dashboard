...
const VerifyTpos = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 p-6 bg-white rounded-xl shadow-md">
      ...
      {tpos.length === 0 ? (
        <p className="text-center text-gray-500">No TPOs Registered Yet.</p>
      ) : (
        tpos.map((tpo, index) => (
          <div key={index} className="border p-4 mb-4 rounded-md text-gray-500">
            ...
            <p><strong>Status:</strong> 
              <span className={`ml-2 ${tpo.status === 'approved' ? 'text-green-600' : tpo.status === 'rejected' ? 'text-red-600' : 'text-yellow-600'}`}>
                {tpo.status}
              </span>
            </p>
            {tpo.status === 'pending' && (
