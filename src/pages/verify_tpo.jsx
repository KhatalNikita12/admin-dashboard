...
const VerifyTpos = () => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5001/api/auth/tpo/pending') // ✅ Make sure this endpoint returns only pending TPOs
      .then((res) => {
        setTpos(res.data || []);
      })
      .catch(() => {
  ...
  const updateStatus = (id, approved) => {
    axios
      .put(`http://localhost:5001/api/auth/tpo/${id}/approve?approved=${approved}`)
      .then(() => {
